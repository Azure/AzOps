...
function Initialize-AzOpsGlobalVariables {
    ...
    process {
        ...
        if ($global:AzOpsInvalidateCache -or -not $global:AzOpsAzManagementGroup -or -not $global:AzOpsSubscriptions) {
            #Get current tenant id
            $TenantId = (Get-AzContext).Tenant.Id
            # Set root scope variable based on tenantid to be able to validate tenant root access if partial discovery is not enabled
