...
$parentModule = 'AzOps'
if (-not $ModuleName) { $ModuleName = 'AzOps.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AzOps'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AzOps.psd1"
$prereqHash = @{
