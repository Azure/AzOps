    ...
    process {
        ...
        if ($scope.Subscription -match ".") {
            ...
            if (($scope.subscription) -and $CurrentAzContext.Subscription.Id -ne $scop.subscription) {
                Write-AzOpsLog -Level Verbose -Topic "pwsh" -Message "Switching Subscription context from $($CurrentAzContext.Subscription.Name)/$($CurrentAzContext.Subscription.Id) to $($scope.subscription)/$($scope.name) "
                try {
        ...
        foreach ($ProviderFeature in $ProviderFeatures) {
            if ($ProviderFeature.FeatureName -and $ProviderFeature.ProviderName ) {
                Write-AzOpsLog -Level Verbose -Topic "pwsh" -Message "Registering Feature $($ProviderFeature.FeatureName) in Provider $($ProviderFeature.ProviderName) namespace"
