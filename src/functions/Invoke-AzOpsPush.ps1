function Invoke-AzOpsPush {
    ...
    process {
        ...
        $deletionList = foreach ($deletion in $deleteSet | Where-Object { $_ -match ((Get-Item $StatePath).Name) }) {
            ...
            if (-not($templateContent.resources[0].type -eq "Microsoft.Authorization/policyAssignments" -or $templateContent.resources[0].type -eq "Microsoft.Authorization/policyExemptions" -or $templateContent.resources[0].type -eq "Microsoft.Authorization/roleAssignments")) {
