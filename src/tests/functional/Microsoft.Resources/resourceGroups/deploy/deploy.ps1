...
try {
    ...
    $script:functionalTestDeploy = New-AzSubscriptionDeployment -Name ($script:resourceType + 'testdeploy') -TemplateFile $script:templateFile -TemplateParameterObject $script:params -Location $script:location -Confirm:$false
