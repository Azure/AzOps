Describe "Scenario - serverfarms" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($global:functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.parameters.appServicePlanName.value).toLower()).json")
    ...
    Context "Test" {
        ...
        It "Resource properties serverFarmId should exist" {
            $script:fileContents.resources[0].properties.serverFarmId | Should -BeTrue
        }
        It "Resource properties kind should exist" {
            $script:fileContents.resources[0].properties.kind | Should -BeTrue
        }
        #endregion Pull Test
