Describe "Scenario - serverfarms" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($filePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.parameters.appServicePlanName.value).toLower()).json")
