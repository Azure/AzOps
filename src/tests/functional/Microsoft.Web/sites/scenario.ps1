Describe "Scenario - sites" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($filePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.outputs.webSiteName.value).toLower()).json")
    ...
    Context "Test" {
        ...
        It "Resource properties siteConfig should exist" {
            $script:fileContents.resources[0].properties.siteConfig | Should -BeTrue
        }
        #endregion Pull Test
