...
Describe "Scenario - bastionHosts" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.parameters.azureBastionName.value).toLower()).json")
    ...
    Context "Test" {
        ...
        It "Resource properties publicIpaddress ID should exist" {
            $script:fileContents.resources[0].properties.ipConfigurations[0].properties.publicIPAddress.id | Should -BeTrue
        }
        It "Resource properties subnet ID should exist" {
            $script:fileContents.resources[0].properties.ipConfigurations[0].properties.subnet.id | Should -BeTrue
        }
        #endregion Pull Test
