...
Describe "Scenario - localNetworkGateways" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.parameters.localNetworkGatewayName.value).toLower()).json")
    ...
    Context "Test" {
        ...
        It "Resource properties addressPrefixes should exist" {
            $script:fileContents.resources[0].properties.localNetworkAddressSpace.addressPrefixes | Should -BeTrue
        }
        It "Resource properties gatewayIpAddress should exist" {
            $script:fileContents.resources[0].properties.gatewayIpAddress | Should -BeTrue
        }
        #endregion Pull Test
