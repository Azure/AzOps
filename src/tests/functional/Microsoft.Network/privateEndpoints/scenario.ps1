...
Describe "Scenario - privateEndpoints" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.parameters.privateEndpointName.value).toLower()).json")
