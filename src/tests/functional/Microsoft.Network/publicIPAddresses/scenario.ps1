Describe "Scenario - publicIPAddresses" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($global:functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.parameters.publicIpAddressName.value).toLower()).json")
    ...
    Context "Test" {
        ...
        It "Resource properties ipAddress should exist" {
            $script:fileContents.resources[0].properties.ipAddress | Should -BeTrue
        }
        #endregion Pull Test
