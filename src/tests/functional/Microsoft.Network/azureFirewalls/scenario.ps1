Describe "Scenario - azureFirewalls" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($global:functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.parameters.azureFirewallName.value).toLower()).json")
