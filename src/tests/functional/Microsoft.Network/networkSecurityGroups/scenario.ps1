Describe "Scenario - networkSecurityGroups" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($global:functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.parameters.networkSecurityGroupName.value).toLower()).json")
    ...
    Context "Test" {
        ...
        It "Resource properties defaultSecurityRules should exist" {
            $script:fileContents.resources[0].properties.defaultSecurityRules | Should -BeTrue
        }
        #endregion Pull Test
