...
Describe "Scenario - networkSecurityGroups" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.parameters.networkSecurityGroupName.value).toLower()).json")
