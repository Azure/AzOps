Describe "Scenario - vaults" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($filePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.parameters.vaultName.value).toLower()).json")
