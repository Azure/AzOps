Describe "Scenario - storageAccounts" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($global:functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.parameters.staName.value).toLower()).json")
    ...
    Context "Test" {
        ...
        It "Resource properties networkAcls should exist" {
            $script:fileContents.resources[0].properties.networkAcls | Should -BeTrue
        }
        It "Resource properties accessTier should exist" {
            $script:fileContents.resources[0].properties.accessTier | Should -BeTrue
        }
        #endregion Pull Test
