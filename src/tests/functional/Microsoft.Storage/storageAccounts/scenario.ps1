...
Describe "Scenario - storageAccounts" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.parameters.staName.value).toLower()).json")
