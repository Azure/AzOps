Describe "Scenario - managementGroups" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($global:functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.parameters.managementGroupId.value).toLower()).json")
