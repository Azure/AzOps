...
Describe "Scenario - virtualMachines" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($functionalTestFilePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.parameters.virtualMachineName.value).toLower()).json")
    ...
    Context "Test" {
        ...
        It "Resource Image Reference should exist" {
            $script:fileContents.resources[0].properties.storageProfile.imageReference | Should -BeTrue
        }
        It "Resource Virtual Machine Size should exist" {
            $script:fileContents.resources[0].properties.hardwareProfile.vmSize | Should -BeTrue
        }
        It "Resource OS disk type should exist" {
            $script:fileContents.resources[0].properties.storageProfile.osDisk.managedDisk.storageAccountType | Should -BeTrue
        }
        It "Resource NIC should exist" {
            $script:fileContents.resources[0].properties.networkProfile.networkInterfaces.id | Should -BeTrue
        }
        It "Resource computerName should exist" {
            $script:fileContents.resources[0].properties.osProfile.computerName | Should -BeTrue
        }
