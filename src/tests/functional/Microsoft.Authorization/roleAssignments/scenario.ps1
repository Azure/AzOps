Describe "Scenario - roleAssignments" {
    ...
    BeforeAll {
        ...
        #region Paths
        $script:path = ($filePaths | Where-Object Name -eq "$($script:resourceProvider)_$($script:resourceType)-$(($script:functionalTestDeploy.value.outputs.roleAssignmentName.value).toLower()).json")
