...
function Initialize-AzOpsRepository {
    ...
    begin {
        ...
        Write-Verbose -Message " - TenantID is $($TenantID)"
    ...
    process {
        ...
        Write-Verbose " - Tenant root management group is: $TenantID"

        if ($PSBoundParameters['Force']) {
            # Force will delete $global:AzOpsState directory
            Write-Warning "Forcing deletion of $global:AzOpsState directory. All artefact will be lost"
        ...
        if ($Rebuild -and (Test-Path -Path $global:AzOpsState)) {
            ...
            Write-Warning "Rebuilding $global:AzOpsState directory by purging all .AzState directories"
        ...
        $StopWatch.Stop()
        Write-Verbose -Message "- Time elapsed: $($stopwatch.elapsed)"
    }
