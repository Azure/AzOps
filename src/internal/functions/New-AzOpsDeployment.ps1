        ...
        if ($scopeFound) {
            ...
            if ($resultsError) {
                ...
                elseif ($resultsErrorMessage -match 'DeploymentWhatIfResourceError' -and $resultsErrorMessage -match "The request to predict template deployment") {
                    ...
                    Set-AzOpsWhatIfOutput -Filename $parameters.TemplateFile -Results ('{0}WhatIf prediction failed with error - validate changes manually before merging:{0}{1}' -f [environment]::NewLine, $resultsErrorMessage)
                }
                else {
                    Write-PSFMessage -Level Warning -String 'New-AzOpsDeployment.WhatIfWarning' -Target $scopeObject -Tag Error -StringValues $resultsErrorMessage
            ...
            elseif ($results.Error) {
                ...
                Set-AzOpsWhatIfOutput -Filename $parameters.TemplateFile -Results $results
            }
