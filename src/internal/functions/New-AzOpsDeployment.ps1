function New-AzOpsDeployment {
    ...
    process {
        ...
        elseif (($scopeObject.StatePath).StartsWith('azopsscope-assume-new-resource_')) {
            ...
            $filename = (Get-Item -Path $addition).Name
            ...
            $pathDirName = (Get-Item -Path $pathDir).Name
            $parentDir = Split-Path -Path $pathDir -Parent
            $parentDirScopeObject = New-AzOpsScope -Path $parentDir | Where-Object {(-not ($_.StatePath).StartsWith('azopsscope-assume-new-resource_'))}
            $parentId = ($resource).properties.details.parent.id
            $parentId = New-AzOpsScope -Scope $parentId | Where-Object {(-not ($_.StatePath).StartsWith('azopsscope-assume-new-resource_'))}
            $expectedDirName = "$($resource.properties.displayName) ($($resource.name))"
            $expectedFileName = ("$($scopeObject.ResourceProvider)_$($scopeObject.Resource)-$($resource.name)" + $(Get-PSFConfigValue -FullName 'AzOps.Core.TemplateParameterFileSuffix')).ToLower()
            ...
            if ($parentDirScopeObject -and $parentId -and $parentDirScopeObject.Scope -eq $parentId.Scope -and $parentDirScopeObject.StatePath -eq $parentId.StatePath -and $parentDirScopeObject.Name -eq $parentId.Name) {
                # Validate directory and file name matches resource information                
                if ($pathDirName -eq $expectedDirName -and $filename -eq $expectedFileName) {
                    # Validate file content metadata matches AzOps
                    if ($resourceScopeFileContent.metadata._generator.name -eq 'AzOps') {
                        Write-PSFMessage -Level Verbose -String 'New-AzOpsDeployment.Root.Processing' -StringValues $defaultDeploymentRegion, $scopeObject -Target $scopeObject
                        $whatIfCommand = 'Get-AzTenantDeploymentWhatIfResult'
                        $deploymentCommand = 'New-AzTenantDeployment'
                    }
                    else {
                        Write-PSFMessage -Level Error -String 'New-AzOpsDeployment.Metadata.Failed' -Target $scopeObject -Tag Error -StringValues $scopeObject
                        throw
                    }
                }
                # Invalid directory or file name
                else {
                    Write-PSFMessage -Level Error -String 'New-AzOpsDeployment.Directory.NotFound' -Target $scopeObject -Tag Error -StringValues $pathDirName, $expectedDirName, $filename, $expectedFileName
                    throw
                }
            }
            # Parent missing
            else {
                Write-PSFMessage -Level Error -String 'New-AzOpsDeployment.Parent.NotFound' -Target $scopeObject -Tag Error -StringValues $parentDir, $addition
                throw
            }
        }       
        else {
